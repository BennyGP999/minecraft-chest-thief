package email.pedersen.chestthief;
...
public class ChestTracker {
    ...
    /**
     * Kiste-positioner der aldrig må stjæles fra — pt. syndikats-base-kister.
     * Nøgle: samme dimensionsstreng som CHESTS_BY_DIMENSION.
     * Opdateres af markSyndicateChest() når en base registreres.
     * findNearest() springer automatisk over alle positioner i dette sæt.
     */
    private static final Map<String, Set<BlockPos>> SYNDICATE_EXCLUDED_BY_DIMENSION = new ConcurrentHashMap<>();
    ...
    public static BlockPos findNearest(Level level, BlockPos origin, double maxRadius, int maxVerticalDist, Set<BlockPos> exclude) {
        ...
        // Hent syndikats-eksklusionssættet for denne dimension.
        // contains() på et Collections.synchronizedSet er trådsikkert som enkeltoperation.
        Set<BlockPos> syndicateExcluded = SYNDICATE_EXCLUDED_BY_DIMENSION.getOrDefault(dim, EMPTY_SET);
        ...
        synchronized (chests) {
            for (BlockPos pos : chests) {
                ...
                if (syndicateExcluded.contains(pos)) continue; // stjæl aldrig fra syndikats-kister
    ...
    /**
     * Markerer en kiste-position som syndikats-kiste, der aldrig må stjæles fra.
     * Kaldes af SyndicateBaseManager.addBase() for hvert kiste-pos i basen.
     * Accepterer ResourceKey<Level> så syndicate-modulet kan kalde metoden
     * uden adgang til et Level-objekt (f.eks. ved server-genstart fra disk).
     * @param dimension dimensionen kisten befinder sig i
     * @param pos       kistens position
     */
    public static void markSyndicateChest(ResourceKey<Level> dimension, BlockPos pos) {
        String dim = dimension.identifier().toString();
        SYNDICATE_EXCLUDED_BY_DIMENSION
    ...
    public static void clearAll() {
        ...
        SYNDICATE_EXCLUDED_BY_DIMENSION.clear();
    }
